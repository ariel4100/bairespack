        ...
        <fieldset>
            ...
            <draggable v-model="gallery" class="row" draggable=".item" @change="log">
                <div class="col-md-4 mb-2 item" v-for="(item,index) in gallery" :key="index" >
                    ...
                    <input type="text" :name="'image['+index+'][image]'" :value="item.image"  class="d-none">
                    <div class="custom-file">
                        <input type="file" @change="previewImage(index,$event)" class="custom-file-input" :name="'image['+index+'][image]'"  lang="es">
                        <label class="custom-file-label"  data-browse="Subir">Imagen</label>
                    </div>
                    <div class="md-form m-0" v-if="!familia">
                        <input type="text" :name="'image['+index+'][title_es]'" :value="item.title_es"  placeholder="Titulo - español" class="form-control">
                    </div>
                    <div class="md-form m-0" v-if="!familia">
                        <input type="text" :name="'image['+index+'][title_en]'" :value="item.title_en" placeholder="Titulo - ingles" class="form-control">
                    </div>
                </div>
            </draggable>
        </fieldset>
